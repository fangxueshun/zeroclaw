    ...
    fn parameters_schema(&self) -> serde_json::Value {
        json!({
            ...
            "properties": {
                ...
                "since": {
                    "type": "string",
                    "description": "Filter memories created at or after this time (ISO 8601, e.g. 2025-03-01T00:00:00Z or 2025-03-01)"
                },
                "until": {
                    "type": "string",
                    "description": "Filter memories created at or before this time (ISO 8601)"
                }
            }
        })
    }
...
mod tests {
    ...
    async fn recall_time_only_returns_entries() {
        ...
        let result = tool
            .execute(json!({"since": "2020-01-01", "limit": 5}))
            .await
            .unwrap();
        assert!(result.success);
        assert!(result.output.contains("Found 1"));
