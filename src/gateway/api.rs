...
fn mask_sensitive_fields(config: &crate::config::Config) -> crate::config::Config {
    ...
    if let Some(dto) = masked.channels_config.dingtalk_outgoing.as_mut() {
        mask_required_secret(&mut dto.sign_token);
        mask_required_secret(&mut dto.outgoing_token);
    }
...
fn restore_masked_sensitive_fields(
    ...
    if let (Some(incoming_ch), Some(current_ch)) = (
        incoming.channels_config.dingtalk_outgoing.as_mut(),
        current.channels_config.dingtalk_outgoing.as_ref(),
    ) {
        restore_required_secret(&mut incoming_ch.sign_token, &current_ch.sign_token);
        restore_required_secret(&mut incoming_ch.outgoing_token, &current_ch.outgoing_token);
    }
    if let (Some(incoming_ch), Some(current_ch)) = (
